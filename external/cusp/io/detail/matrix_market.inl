...
void read_coordinate_stream(cusp::csr_matrix<IndexType,ValueType,cusp::host_memory>& csr, Stream& input, const matrix_market_banner& banner)
{
    ...
    //Bug fix: Swap() at the end of the following read_coordinate_stream is not implemented for CooView
    //Copying to CSR costs some more memory operations but it is the correct way in order to read symmetric
    //matrices
...
//                cusp::make_array1d_view(csr.values));
    cusp::coo_matrix<IndexType,ValueType,cusp::host_memory> coo(num_rows, num_cols, num_entries);
